...
def search(person: dict) -> list[dict]:
    """Find Notion Marketplace profiles via Google Search via Apify."""
    if not APIFY_TOKEN:
        return []

    query = f'"{person["name"]}" site:notion.so/marketplace'
    ...
    try:
        ...
        print(f"  [notion_mp] Apify error: {e}")
        try:
            time.sleep(5)
            resp = requests.post(
                RUN_URL,
            ...
            print(f"  [notion_mp] retry failed: {e2}")
            return []
    ...
    for item in items[:3]:
        ...
        for r in organic[:3]:
            url = r.get("url", "")
            if "notion.so/marketplace" in url:
                results.append({
                    "url": url,
                    "title": r.get("title", ""),
